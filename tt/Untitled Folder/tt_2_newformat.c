...
for(i=0;i<(number_of_slot+lab_slot);i++)
        {
        	printf("%s ",&ts_new[i].time_slot);
        	if(i<4)
        	{
			
        	for(j=0;j<slot_size;j++)
        	{
        	printf("%d ",ts_new[i].day[j]);
        	printf("%d ",ts_new[i].time[j]);
			}
	     }
	    else
	     {
...
room_occupancy[0][0]="Room No.";room_occupancy[0][1]="Slot occupied";room_occupancy[0][2]="Slot Free ";
